...
*Snapshot 2026-07-21 (round 17) — refresh by invoking `/dave-core:dashboard` in this repo.*
...
1. **Walk round 17 live** (Pages ~10 min; hard-refresh first): on your test account, picker → "+ Start a new project" → the consent card → create → you should land IN the new wizard with "New project (setting up)" in the picker and "Getting ready" as the day chip. Walk wizard (try the context-box edit + a chip) → Day-0 get-ready card → "Start my Freedom Power Hour and Day 1 now →" → tick tool 1 and watch the chip flip to Day 1. Phone + desktop. NOTE: your OLD projects keep their day counting exactly (legacy-grandfathered); only projects completing setup from now on get the Getting-ready behavior.
2. **The new-project email is now OFF by default.** To turn it back on someday: Accelerator sheet → "AI Settings" tab → `NEW_PROJECT_EMAIL` = `on` (rerun "Set up AI Settings tab" from the menu if the row is missing). Before any email coaching resumes, the Accelerator's day references must be re-anchored to `day1Date` (recorded in the plan doc).
3. Walk rounds 13–16 items if not yet done: progress toggle, More help & tools, milestone journey (`fh preview` → milestone / decided). Your real account shows the gold card once your scores hold 7+/7+ for three logged days.
