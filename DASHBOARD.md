...
1. **Arm the diag tools (2 minutes, once):** Terminal → `cd ~/Desktop/coding_projects/freedom_tracker_gateway && node tools/diag.js keys` → it prints two keys. Open the [Gateway sheet](https://docs.google.com/spreadsheets/d/1QkjvMI3_HfrKBEJDGWrt8XI2iqX_DUTtfHPWZHv0eZI/edit) → Extensions → Apps Script → run `gateway_setAdminKey()` → paste the gateway key. Open the [Accelerator sheet](https://docs.google.com/spreadsheets/d/1SgagSNRrGHQ96Ea04JC6KPJRxCqsXKAwM-xtlyJuHFI/edit) → Extensions → Apps Script → run `accel_setAdminKey()` → paste the accel key. From then on Claude reads logs/registry/Master rows without you.
2. **Retry the create** (hard-refresh first, Pages ~10 min): picker → + Start a new project → Create. Either it works now and lands in the wizard, or the card prints the exact failure reason — screenshot or read it out, or just say "run the diag" once step 1 is done.
...
| **Archive control** | goal & plan room, >1 active project | "Nothing is deleted" note + gray "Archive this project →"; archiving lands on the account default | `?s=multi` → tap goal line |
| **Archived-projects room** | picker "Archived projects… (N)" (only when any exist) | restore list, "Bring it back →" lands ON the restored project | `?s=multi` (ships with one pre-archived) |
