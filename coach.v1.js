/* ============================================================
   FREEDOM COACH LOADER v1  ·  host on GitHub Pages as coach.v1.js
   ------------------------------------------------------------
   A standalone in-lesson widget. Drop it on any "AI Coach" lesson:

     <div id="freedom-coach"></div>
     <script src="https://USER.github.io/freedom-tracker/coach.v1.js"><\/script>

   It is its own thing, separate from the tracker loader, but it
   speaks to the SAME Gateway and reuses the SAME activation: if the
   student has opened their tracker once, the shared token in
   localStorage means the coach is already signed in. No second setup.

   WHAT IT DOES (two moves)
     1. RECOMMEND  "Recommend my next move" -> the coach reads where
        they are and hands back ONE ready-to-paste prompt for the
        right tool, with one-click copy.
     2. CHAT + PROPOSE  A short back-and-forth. When the conversation
        surfaces a real update, the coach shows a review card with the
        exact before/after text. One tap writes it to their tracker.

   SAFETY (handled server-side, surfaced here)
     - The coach only ever writes the three richness fields on the
       current day. Never scores, checkboxes, the UB, the jumpstart.
     - The review card shows the exact final text. Apply writes that
       verbatim. Nothing the student did not see.
     - A distressed message is caught in the Gateway and comes back as
       a calm support reply, with no prompt and no proposal. This
       widget renders that reply gently and does nothing else.

   PAIR WITH: CoachGateway.gs + CoachAi.gs in the Gateway project.
   ============================================================ */
   (function () {
    'use strict';
     ...
     * The coach's actual replies come from the AI in Dave's voice;
     * this is only the surrounding UI text.
     * ============================================================ */
    var COPY = {
    ...
    function onApply(proposal, cardEl, btn) {
      ...
      callGateway({ action: 'coachApply', projectId: state.activeProjectId, updates: updates })
        .then(function (data) {
          ...
          // Mirror the new values into local context so later prompts see them.
          for (var i = 0; i < proposal.updates.length; i++) {
            // nothing to mirror into UB; richness lives server-side only
          }
    ...
    function pushCoach(text, opts) {
      ...
      // Pending "Thinking..." bubbles are shown but not stored in history.
