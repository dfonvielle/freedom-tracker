/* ============================================================
   FREEDOM TRACKER LOADER v2 — host on GitHub Pages.

   v2 changes:
   - Magic link capture is IFRAME-AWARE: ?ag_token=... is read
     from this frame's URL, then the PARENT page's URL (embeds
     run in a same-origin iframe, so the lesson URL lives one
     level up), then document.referrer. This is why v1's magic
     link never fired.
   - Stale-token recovery: if the Gateway rejects the stored
     token (e.g. a different student is now logged in on this
     computer), the loader clears the stale token and shows the
     Activate screen with a friendly message — never another
     student's tracker, never a dead end.
   - Testing helpers: add ?ag_reset=1 to any lesson URL to wipe
     stored token + identity cache; or run AG_FT_RESET() in the
     console.

   Per-lesson stub stays the same:
     <div id="freedom-tracker"></div>
     <script src="https://YOUR-USERNAME.github.io/freedom-tracker/loader.v2.js"></script>
   (Rename the hosted file to loader.v2.js and update the stub
   so old cached copies can't linger.)
   ============================================================ */
(function () {
  'use strict';
  ...
  // ============================================================
  // URL param reading — iframe-aware
  // ============================================================
  // Collects candidate URLs: this frame, the parent lesson page
  // (same-origin on the school domain), and the referrer.
  function candidateUrls() {
    var urls = [];
    try { urls.push(window.location.href); } catch (e) {}
    try {
      if (window.parent && window.parent !== window) {
        urls.push(window.parent.location.href);   // throws if cross-origin
      }
    } catch (e) {}
  ...
  function captureMagicLinkToken() {
    ...
    if (t) writeLS(LS.token, t);   // overrides any previous student's token
  }

  function handleResetParam() {
    if (readParam('ag_reset')) resetStorage();
  }
  ...
  // Console helper for testing: AG_FT_RESET()
  try {
  ...
  function boot() {
    ...
    handleResetParam();        // ?ag_reset=1 wipes storage first
    captureMagicLinkToken();   // ?ag_token=... overrides stored token
  ...
  // ============================================================
  // Identity (fresh fetch first; cache only as fallback)
  // ============================================================
  ...
  function loadState(root) {
    ...
    callGateway({ action: 'state', projectId: state.activeProjectId })
      .then(function (data) {
        if (!data.ok) {
          // Stale or wrong token for the current identity (e.g. a
          // different student on a shared computer). Clear it and
          // send them to Activate — never a dead end, never
          // someone else's tracker.
          resetTokenOnly();
          return renderPairing(root,
  ...
  // ============================================================
  function renderPairing(root, notice) {
