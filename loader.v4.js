/* ============================================================
   FREEDOM TRACKER LOADER v4 — host on GitHub Pages as
   loader.v4.js (new filename; update lesson stubs).

   Stub (per lesson):
     <div id="freedom-tracker" data-view="full"></div>
     <script src="https://USER.github.io/freedom-tracker/loader.v4.js"></script>

   Views: full | day:N | today | day1guide | progress |
          goalplan | evaluate          (default: full)
   The 4-step setup wizard intercepts every view until done.

   v4 (per V4-Spec.md):
   - COPY object below: ALL loader-side student-facing text.
     (Field labels / plan / score questions / Day 0 teaching
     text come from the Gateway's COPY and arrive in payloads.)
   - Legibility pass: 16px+ inputs (no iOS zoom), 15.5px bold
     labels, everything left-aligned, 44px buttons, big checks.
   - 4-step wizard (UB textarea -> baseline -> jumpstart time
     -> Goal & Plan + "Start My Sprint"). TBD/blank rejected;
     scores must be whole numbers 0-10 (client AND server).
   - Goal & Plan = the ONLY place to edit UB + jumpstart time.
   - day1guide + Daily Check-In guidance: personalized
     copy-prompt (student's UB baked in) with clipboard copy
     + manual-copy fallback.
   - evaluate view: locked (encouraging) before Day 7 scores;
     unlocked shows deltas + tier recommendation.
   - PERFORMANCE: cache-then-refresh (instant render from
     localStorage, background refresh; re-render skipped if the
     student is mid-typing) + optimistic saves.
   - All v2/v3 behaviors retained: iframe-aware magic link
...
(function () {
  ...
  var COPY = {
    ...
    WIZ1_LABEL: 'For this one-week sprint, I want to break more free from the following unwanted behavior (UB):',
    ...
    WIZ2_TITLE: 'Your baseline \u2014 before any rewiring',
    WIZ2_SUB: 'In 7 days you\u2019ll look back at these numbers as your proof of change. Be honest \u2014 low scores now make the climb more satisfying.',
    ...
    WIZ3_SUB: 'Each morning of the sprint you\u2019ll do just 30 seconds to 2 minutes of rewiring \u2014 your Happiness & Success Jumpstart. Anchor it to something you already do every morning. For example: while brushing your teeth, while the coffee brews, in the shower, while getting dressed.',
    WIZ3_LABEL: 'I\u2019ll do my morning Jumpstart at this time and place:',
    ...
    GOAL_LABEL: 'Over the next 7 days, I want to break more free from:',
    GOAL_JS_LABEL: 'Each morning I\u2019ll do 30 seconds\u20132 minutes of rewiring (H&S Jumpstart) at this time and place:',
    GOAL_BUTTON: 'Update',
    GOAL_SAVED: 'Updated \u2713',
    GOAL_PREFIX: 'YOUR GOAL: greater freedom from ',
    GOAL_SUFFIX: ' \u2014 proven to yourself in 7 days.',

    D1G_TITLE: 'Day 1 Guidance \u2014 Your Freedom Power Hour',
    D1G_INTRO: 'For about one focused hour, you\u2019ll use these four AI tools, in this exact order. Each one rewires from a different angle:',
    D1G_TOOLS: ['No-Brainer Willpower Eliminator', 'Minimalist Plan', 'Feel Good Start', 'Create Joyous Chaos'],
    D1G_AFTER: 'Optional afterwards: the Withdrawal Helper, if withdrawal is a concern. Then come back to the Day 1 Check-In to log everything.',
    D1G_PROMPT_LABEL: 'Your starter prompt \u2014 paste this into each tool:',
    D1G_PROMPT: 'I want to make it easier and more enjoyable to no longer do the following behavior: {UB}',
    ...
    DAILY_GUIDE_TITLE: 'Your daily rewiring routine',
    DAILY_GUIDE_TEXT: 'Each day: do your morning Happiness & Success Jumpstart, spend 5\u201320 minutes with the Rapid Behavioral Freedom tool, then check in below.',
    DAILY_PROMPT_LABEL: 'Starter prompt for the Rapid Behavioral Freedom tool:',
    DAILY_PROMPT: 'I want to break more free from the following behavior: {UB}. I want to make it easier and more enjoyable to not do it anymore. Here\u2019s more about what I\u2019m dealing with right now:',
    DAILY_PROMPT_TIP: 'Paste it in, then talk out whatever\u2019s going on \u2014 brain dumps work great.',

    GROUNDING: 'Here\u2019s what I did today to break more free from: ',
  ...
  var LS = { identity: 'ag_ft_identity', token: 'ag_ft_token', cache: 'ag_ft_cache' };
    ...
    try {
      localStorage.setItem(cacheKey(), JSON.stringify({
        ...
        snapshot: {
          ...
          setup: state.setup, plan: state.plan, scoreQuestions: state.scoreQuestions,
  ...
  function boot() {
    ...
    } else if (['today', 'progress', 'goalplan', 'full', 'setup', 'day1guide', 'evaluate'].indexOf(dv) >= 0) {
  ...
  function route() {
    ...
    if (state.view === 'day1guide') return renderDay1Guide();
  ...
  function wizardStepJumpstart() {
    rootEl.innerHTML =
      '<div class="ft-card">' + wizardDots(3) +
        ...
        '<p class="ft-sub">' + esc(COPY.WIZ3_SUB) + '</p>' +
  ...
  function goalPlanReadOnlyHtml() {
    ...
    var html = '<h3 class="ft-center-h">' + esc(COPY.GOAL_TITLE) + '</h3>' +
      '<div class="ft-goal">' + esc(COPY.GOAL_PREFIX) + '<strong>' + esc(ub) + '</strong>' + esc(COPY.GOAL_SUFFIX) + '</div>';
    if (state.setup.jumpstart) {
      html += '<div class="ft-goal">Every morning: 30 seconds\u20132 minutes of rewiring \u2014 <strong>' + esc(state.setup.jumpstart) + '</strong>.</div>';
    }
    html += '<div class="ft-plan">' + esc(state.plan) + '</div>';
    return html;
  }
  ...
  function goalPlanEditHtml() {
    return '<h3 class="ft-center-h">' + esc(COPY.GOAL_TITLE) + '</h3>' +
      ...
      '<div class="ft-plan" style="margin-top:16px;">' + esc(state.plan) + '</div>';
  }
  ...
  // ============================================================
  // View: day:N / today / day1guide / progress / evaluate
  // ============================================================
  ...
  function dailyGuideHtml() {
    ...
    return '<div class="ft-guide">' +
      '<div class="ft-label">' + esc(COPY.DAILY_GUIDE_TITLE) + '</div>' +
      '<p class="ft-body-text">' + esc(COPY.DAILY_GUIDE_TEXT) + '</p>' +
  ...
  function renderDay1Guide() {
    ...
    for (var i = 0; i < COPY.D1G_TOOLS.length; i++) toolsHtml += '<li>' + esc(COPY.D1G_TOOLS[i]) + '</li>';
    toolsHtml += '</ol>';

    renderShell(
      '<h4>' + esc(COPY.D1G_TITLE) + '</h4>' +
  ...
  function renderEvaluateView() {
    ...
    callGateway({ action: 'evaluation', projectId: state.activeProjectId })
      .then(function (data) {
        ...
        deltasHtml += '<div class="ft-traj" style="margin-top:6px;"><span class="ft-traj-label">Day 7 average:</span> ' + esc(data.average) + ' / 10</div></div>';

        setBody(
          '<h4>' + esc(data.copy.tierTitle) + '</h4>' +
          deltasHtml +
          '<p class="ft-body-text">' + esc(data.copy.tierText) + '</p>' +
          '<p class="ft-body-text">' + esc(data.copy.sendoff) + '</p>'
        );
      })
      .catch(function (err) { setBody(badMsg(String(err))); });
  }
  ...
  function renderDayCard(dd, opts) {
    ...
    if (dd.scores) {
      html += '<div class="ft-dayscores"><div class="ft-label" style="margin-bottom:10px;">' + esc(dd.scores.label) + '</div>';
  ...
  function injectStyles() {
    ...
    var css =
      ...
      '#freedom-tracker .ft-chips{display:flex;flex-wrap:wrap;gap:7px;margin:12px 0 16px 0;}' +
      ...
      '#freedom-tracker .ft-field{margin-bottom:16px;text-align:left;}' +
      ...
      '#freedom-tracker button{background:#1f6f5c;color:#fff;border:none;border-radius:8px;padding:12px 18px;font-size:16px;font-weight:700;cursor:pointer;width:100%;margin-top:6px;min-height:44px;}' +
      ...
      '#freedom-tracker .ft-goal{background:#eef6f3;border:1px solid #bcd9cf;border-radius:8px;padding:12px 14px;font-size:15.5px;margin-bottom:12px;text-align:left;}' +
      '#freedom-tracker .ft-plan{white-space:pre-wrap;font-size:15px;margin-bottom:16px;text-align:left;}' +
