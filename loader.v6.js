/* ============================================================
   FREEDOM TRACKER LOADER v6 — host on GitHub Pages as
   loader.v6.js (new filename; update lesson stubs).
   Stub (per lesson):
     <div id="freedom-tracker" data-view="full"></div>
     <script src="https://USER.github.io/freedom-tracker/loader.v6.js"><\/script>

   Pairs with Gateway v6.1.

   WHAT CHANGED FROM v5
   ------------------------------------------------------------
   1. IDENTITY = the Systeme.io account id (the integer), not slug.
      fetchSystemeIdentity now reads userData.id as accountId, and
      callGateway sends accountId (not slug). The slug is dropped.

   2. CACHE VERSIONS BUMPED (ag_ft_identity_v6, ag_ft_cache_v6) so a
      returning student never loads a stale slug-shaped identity or a
      v5-shaped state snapshot after the switch. The token key is kept;
      stale tokens self-heal via recover (see #3).

   3. TOUCHLESS ONBOARDING. When there is no token, the loader calls
      the Gateway 'recover' action FIRST (account id + email from the
      logged-in session) and, if found, stores the returned token
      inline and proceeds — no email, no paste. The "enter your code"
      screen is now a LAST RESORT, shown only if recover finds nothing
      or the session identity is unavailable. A failed 'state' call
      (e.g. a migrated user holding an old token) also routes through
      recover before pairing, so stale tokens heal silently.

   4. INLINE TOKEN PERSISTENCE. Any response that carries a token
      (recover, state, activate) is persisted to localStorage, so the
      present device activates without waiting on the welcome email.

   5. URL TOKEN STRIPPING. After capturing ?ag_token=, it is removed
      from the visible URL (history.replaceState) so the strong
      credential does not linger in history/referrer.

   6. READ-ONLY AFTER WINDOW. The Gateway returns a per-project
      'writable' flag and rejects out-of-window writes with
      windowClosed. The loader shows a read-only banner and disables
      save when a project is past its editing window.

   All v2-v5 behaviors retained: iframe-aware identity, cache-then-
   refresh, optimistic saves, the 4-step wizard, all day/guidance/
   scores/evaluation/goalplan views, copy buttons, project picker.
   ============================================================ */
(function () {
  'use strict';
  // ============================================================
  ...
  // Cache versions bumped to v6 so v5/slug-shaped data is ignored.
  ...
  // ============================================================
  // TOUCHLESS RECOVER
  // Find this logged-in student by account id + email, get their token
  // back inline, and proceed — no email, no paste. Falls back to the
  // setup-in-progress card (recover notFound) or the manual activation
  // screen (no session identity / other failure).
  // ============================================================
